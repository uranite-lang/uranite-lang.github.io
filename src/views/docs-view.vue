        ...
        <div class="lg:sticky lg:top-24 space-y-4">
          ...
          >
            ...
            <div class="border-t border-gray-200 dark:border-gray-800 pt-4">
              <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Contents</label>
...
useHead({
  title: computed(() => path.value ? `${path.value} — Uranite Docs` : 'Documentation — Uranite'),
  meta: [
    { name: 'description', content: computed(() => path.value ? `Uranite documentation: ${path.value}` : 'Uranite language documentation, guides, and references.') },
    { property: 'og:title', content: computed(() => path.value ? `${path.value} — Uranite Docs` : 'Documentation — Uranite') },
    { property: 'og:description', content: 'Uranite language documentation, guides, and references.' },
...
async function loadTags() {
  try {
    tags.value = await store.dispatch('loadTags')
  } catch {}
}
...
async function loadTree() {
  ...
  try {
    ...
    const data = await store.dispatch('loadDocsTree', version.value)
    if (data.truncated) {
      store.commit('setError', 'Documentation tree was truncated by GitHub. Some files may be missing.')
    }
    tree.value = buildTree(data.tree || [], 'docs')
  } catch (err) {
...
async function loadContent() {
  ...
  try {
    store.commit('clearError')
    content.value = await store.dispatch('loadDocContent', { version: version.value, path: path.value })
  } catch (err) {
    store.commit('setError', err)
    content.value = ''
  } finally {
...
function onSelect(filePath) {
  router.push(`/docs/${version.value}/${filePath}`)
